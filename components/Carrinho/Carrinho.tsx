// components/Carrinho/Carrinho.tsx
'use client';
...
interface CarrinhoProps {
    ...
    onPurchase: (student: boolean, coupon: string) => Promise<void>; 
...
export const CarrinhoPuro: React.FC<CarrinhoProps> = ({ cart, onRemoveFromCart, onPurchase, purchaseStatus }) => {
    ...
    const { totalCusto, totalComDesconto } = useMemo(() => {
        ...
        return { 
            ...
            totalComDesconto: precoFinal.toFixed(2) 
        };
    }, [cart, isDeisiStudent, coupon]);

    const handleBuy = () => {
        onPurchase(isDeisiStudent, coupon);
    };
    ...
    return (
        <div className="cart-box relative">
            ...
            <div className="cart-summary">
                ...
                <div className="cart-discount-options">
                    <label className="discount-label flex items-center">
                        <input 
                            ...
                            checked={isDeisiStudent} 
                            onChange={(e) => setIsDeisiStudent(e.target.checked)} 
                        />
                    ...
                    <input 
                        type="text" 
                        placeholder="Cupão de Desconto (ex: DEISI20)" 
                ...
                <button
                    className={`buy-button ${cart.length === 0 || purchaseStatus.startsWith('A') ? 'buy-disabled' : ''}`}
                    disabled={cart.length === 0 || purchaseStatus.startsWith('A')}
                    onClick={handleBuy} 
                >
                    {purchaseStatus.startsWith('A') ? 'A Processar...' : 'Comprar'}
                </button>
